...
const VendorItemEditPage = () => {
  ...
  useEffect(() => {
    let isMounted = true;
    Promise.all([getVendorList(id), getVendorModel(itemId), listVendorModels({ vendorId: id })])
      .then(([vendorRecord, itemRecord, models]) => {
        if (!isMounted) return;
        setVendor(vendorRecord);
        setVendorItems(models);
        setItemForm({
          item_name: String(itemRecord.item_name),
          rate: String(itemRecord.rate),
        });
      })
      .catch(() => {
        if (!isMounted) return;
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await updateVendorModel({ id: itemId, payload });
      navigate(`/vendors/${id}`);
    } catch (requestError) {
      const apiMessage =
        ...
        <div className="content-card">
          <div className="section-head">
            <div>
              ...
              <p>Update the item rate for this vendor.</p>
            </div>
...
export default VendorItemEditPage;
